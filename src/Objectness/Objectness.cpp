...
void Objectness::generateTrianData_BigData()
{
	...
	const int NUM_NEG_BOX = 2; //100,// Number of negative windows sampled from each image
...
#pragma omp parallel for
	for (int i = 0; i < NUM_TRAIN; i++)	{
		...
		for (int k = 0; k < NUM_NEG_BOX; k++){
			...
			Vec4i bb(min(x1, x2), min(y1, y2), max(x1, x2), max(y1, y2));
			//fixed by chigo-20160129
			if (maxIntUnion(bb, _voc.gtTrainBoxes[i]) < 0.3)	//THRESHOLD:0.3, 0.5
				xN.push_back(getFeature(im3u, bb));
		}
	}
...
int Objectness::gtBndBoxSampling(const Vec4i &bbgt, vector<Vec4i> &samples, vecI &bbR)
{
	...
	for (int h = hMin; h <= hMax; h++) for (int w = wMin; w <= wMax; w++){
		...
		if (DataSetVOC::interUnio(bb, bbgt) >= 0.7){	//THRESHOLD:0.7,0.5
...
void Objectness::Predict( string imgfile, int numDetPerSize)
{
	...
	{	
		cout<<"Can't open " << imgfile.c_str() << endl;
...
void Objectness::evaluatePerImgRecall(const vector<vector<Vec4i> > &boxesT, CStr &saveName, const int NUM_WIN)
{
	...
	int idx[10] = {1, 10, 20, 50, 100, 500, 1000, 2000, 3000, 5000};
	for (int i = 0; i < 10; i++){
		if (idx[i] > NUM_WIN)
			continue;
