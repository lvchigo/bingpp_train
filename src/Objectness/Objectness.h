...
public:
	...
	void getObjBndBoxesEva(Mat &img3u, ValStructVec<float, Vec4i> &valBoxes, ValStructVec<float, Vec4i> &evaBoxes, vecI &sz, int numDetPerSize = 120);
	void Predict( string imgfile, int numDetPerSize);
	...
	void getObjBndBoxesForTestFast(vector<vector<Vec4i> > &boxes, int numDetPerSize = 100);
	void getObjBndBoxesForTrainsEva(vector<vector<Vec4i> > &boxesTrains, int numDetPerSize = 100);
	...
	void analysePerImg(vector<vector<Vec4i> > &boxesTests, vector<vector<Vec4i> > &boxesEvas, vector<vecI> &sz);
	void evaluateShift(vector<vector<Vec4i> > &boxesTests, vector<vector<Vec4i> > &shift);
	...
	static Mat trainSVM(const vector<Mat> &pX1f, const vector<Mat> &nX1f, int sT, double C, double bias = -1, double eps = 0.01, int maxTrainNum = 10000000);
...
public: // For illustration etc.
	...
	void evaluatePAMI12(CStr &saveName = "MAMI12.m");
	void evaluateIJCV13(CStr &saveName = "IJCV13.m");
	void evaluateECCV14(CStr &saveName = "ECCV14.m");
	...
	void illustrateGT();  //Illustrate the ground truth in images
